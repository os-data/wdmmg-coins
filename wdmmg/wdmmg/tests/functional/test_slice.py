...
class TestSliceController(TestController):
    ...
    def test_view(self):
        response = self.app.get(url(controller='slice', action='view', name_or_id='cra'))
    ...
    def test_accounts(self):
        response = self.app.get(url(controller='slice', action='accounts', name_or_id='cra'))
        assert "'cra'" in response
    ...
    def test_accounts_paginate(self):
        response = self.app.get(url(controller='slice', action='accounts',
            name_or_id='cra', items_per_page=2))
    ...
    def test_accounts(self):
        response = self.app.get(url(controller='slice', action='transactions', name_or_id='cra'))
        assert "'cra'" in response
